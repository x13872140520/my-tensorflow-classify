...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load("C:/Users/张/Desktop/fix/newdata/train",
"C:/Users/张/Desktop/fix/newdata/val",
224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/mobilenet_flowers2.h5")
    # model.summary()
    # 测试
...
def test_cnn():
    # todo 加载数据, 修改为你自己的数据集的路径
    train_ds, test_ds, class_names = data_load("E:/project/newdata/train",
                                              "E:/project/newdata/val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/cnn_follwers.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
