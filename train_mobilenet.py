...
def train(epochs):
    ...
    tf.saved_model.save(model, saved_model_path, signatures=create_signature(model))
    # 记录结束时间
    end_time = time()
...
def create_signature(model):
    ...
    # 创建新的签名
    signature = predict_image.get_concrete_function(
        image_b64=tf.TensorSpec([None], dtype=tf.string, name="image_b64")
    )
    return {"predict_image": signature}
